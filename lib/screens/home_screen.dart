...
class _HomeScreenState extends State<HomeScreen>
  ...
  void initState() {
    ...
    // Keep color updates in sync with tab movement (tap and swipe) with no lag.
    _tabController.animation?.addListener(() {
      if (!mounted) return;
      final nextIndex = _tabController.animation!.value.round().clamp(0, 1);
  ...
  void _openSearch(DebtProvider debtProvider) {
    showSearch(context: context, delegate: DebtSearchDelegate(debtProvider));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isLendTab = _activeTabIndex == 0;
    final appBarColor = isLendTab
        ? (isDark
              ? const Color(0xFFB54758)
              : const Color.fromARGB(255, 230, 68, 87))
